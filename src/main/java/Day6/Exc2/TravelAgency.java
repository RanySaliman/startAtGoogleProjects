...
public class TravelAgency {
    ...
    public TravelAgency(HashMap<VehicleType, Integer> demand) {
        SingletonVehicle.getInstance();
        controller = new HashMap<>();

        demand.forEach((key, value) -> {
            availableVehicles += value;
            for (int i = 0; i < value; i++) {
                controller.put(SingletonVehicle.createVehicle(key), null);
            }
        });
    }
    ...
    public void assign(Passenger passenger) {
        ...
        for (Map.Entry<Vehicle, Passenger> set : controller.entrySet()) {
            if (set.getKey().getType() == passenger.getFavoriteVehicle() && set.getValue() == null) {
    ...
    public void print() {
        controller.entrySet().forEach(set -> {
            if (set.getValue() == null) {
                ...
                System.out.println(set.getKey().getType() + " " + set.getValue().getFavoriteVehicle());
            }
        });
    }
}
